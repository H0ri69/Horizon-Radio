                ...
                <div className="p-10 md:p-14 space-y-12">
                    ...
                    <header className="flex justify-between items-center pb-8 border-b border-white/5">
                        <div>
                            ...
                            <p className="text-lg text-secondary text-white/50">Configure your Hori-s.FM AI Host experience</p>
                        </div>
                        ...
                        >
                            <div className={`w-3 h-3 rounded-full ${settings.enabled ? 'bg-green-500 shadow-[0_0_12px_rgba(34,197,94,0.6)]' : 'bg-red-500'}`}></div>
                    ...
                    <main className="space-y-12 text-white">
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                {[
                                    ...
                                    >
                                        <div className="font-medium text-xl group-active:scale-95 transition-transform">{lang.label}</div>
                                    </button>
                                ))}
                            </div>
                        </section>
                        ...
                        <section>
                            <h2 className="text-xl font-semibold text-white mb-6 flex items-center gap-3">
                                <span className="text-indigo-400 font-mono text-base opacity-80">01</span> Voice Model
                            </h2>
                                ...
                                {VOICE_PROFILES.map(profile => (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-4">
                                            <span className={`text-xl font-medium ${settings.voice === profile.id ? 'text-white' : 'text-white/70 group-hover:text-white'}`}>
                                                {profile.name}
                                            </span>
                                            {settings.voice === profile.id && (
                                        ...
                                        <div className="flex items-center gap-3 text-sm text-white/40">
                                            <span className="bg-white/5 px-3 py-1 rounded-md border border-white/5">{profile.tone}</span>
                                            <span className="bg-white/5 px-3 py-1 rounded-md border border-white/5">{profile.emotion}</span>
                                        </div>
                                    </button>
                                ))}
                            </div>
                        ...
                        <section>
                            <h2 className="text-xl font-semibold text-white mb-6 flex items-center gap-3">
                                <span className="text-indigo-400 font-mono text-base opacity-80">02</span> Broadcast Style
                            </h2>
                            ...
                            {/* Custom Prompt Input */}
                            <div className={`transition-all duration-500 ease-in-out overflow-hidden ${settings.style === DJStyle.CUSTOM ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0'}`}>
                        ...
                        <section>
                            <h2 className="text-xl font-semibold text-white mb-6 flex items-center gap-3">
                                <span className="text-teal-400 font-mono text-base opacity-80">03</span> Dual DJ Mode
                            </h2>
                            
                            <div 
                                className={`rounded-3xl border transition-all duration-200 overflow-hidden ${settings.dualDjMode ? 'bg-white/10 border-teal-500/30 ring-1 ring-teal-500/20' : 'bg-white/5 border-white/5'}`}
                            >
                                <div className="p-6 flex items-center justify-between cursor-pointer" onClick={() => saveSettings({ ...settings, dualDjMode: !settings.dualDjMode })}>
                                    <div>
                                        <div className="font-medium text-xl text-white">Enable Co-Host</div>
                                        <div className="text-base text-white/50 mt-1">Two DJs will banter during transitions</div>
                                    </div>
                                    <div className={`w-16 h-8 rounded-full p-1 transition-colors ${settings.dualDjMode ? 'bg-teal-500' : 'bg-white/10'}`}>
                                        <div className={`w-6 h-6 rounded-full bg-white shadow-sm transition-transform ${settings.dualDjMode ? 'translate-x-8' : 'translate-x-0'}`} />
                                    </div>
                                </div>

                                {settings.dualDjMode && (
                                    <div className="px-6 pb-6 pt-0 border-t border-white/5 mt-2">
                                        <h3 className="text-sm font-bold text-white/40 uppercase tracking-wider mb-4 mt-6">Select Co-Host Voice</h3>
                                        <div className="grid grid-cols-2 gap-4">
                                            {VOICE_PROFILES.filter(p => p.id !== settings.voice).map(profile => (
                                                ...
                                                >
                                                    ...
                                                    {settings.secondaryDjVoice === profile.id && <div className="w-2 h-2 rounded-full bg-teal-400"></div>}
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                    ...
                    <footer className="pt-8 border-t border-white/5 flex justify-between items-center text-sm text-white/30">
                        ...
                        <div className={`transition-all duration-300 font-medium ${status ? 'opacity-100 text-green-400' : 'opacity-0'}`}>
                            SETTINGS SAVED
                        </div>
                    </footer>
                </div>

...
import {
    ...
    Play, Loader2, Smartphone, Mic, Scan, Link as LinkIcon
} from 'lucide-react';
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 font-sans">
            ...
            >
                ...
                <div className="p-8 md:p-10 border-b border-white/5 flex justify-between items-center modal-header z-20">
                    ...
                    <div className="flex items-center gap-4">
                        {/* Mode Indicator */}
                        <div className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border transition-all ${mode === 'REMOTE' ? 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400' : 'bg-white/5 border-white/10 text-white/40'}`}>
                            ...
                            <div className="space-y-10">
                                <motion.section variants={itemVariants} className="h-full flex flex-col">
                                    ...
                                    <div className="modal-section rounded-3xl p-8 border border-white/5 flex-1 flex flex-col items-center justify-center bg-white/5 text-center relative overflow-hidden">
                                        ...
                                        <div className={`absolute top-6 right-6 flex items-center gap-2 px-3 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-widest transition-all
                                            ${remoteStatus.startsWith('CALLER:')
                                                ? 'bg-green-500/10 border-green-500/30 text-green-400'
                                                : 'bg-white/5 border-white/10 text-white/40'}`}>
                                            <div className={`w-1.5 h-1.5 rounded-full ${remoteStatus.startsWith('CALLER:') ? 'bg-green-500 animate-pulse' : 'bg-white/40'}`} />
                                            {remoteStatus.startsWith('CALLER:') ? 'Guest Connected' : 'Waiting for Device'}
                                        </div>
                                        ...
                                        <p className="text-white/50 text-sm mb-6 max-w-[80%]">
                                            Use your phone to fill out your details and join the call remotely with end-to-end audio.
                                        </p>
                                        ...
                                        <div className="mt-8 text-center pb-8">
                                            <div className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 mb-2">Secure Link ID</div>
                                            <div className="font-mono text-xl text-white/80 tracking-widest">{hostId}</div>
                                        </div>
                        ...
                        <motion.div variants={itemVariants} className="pt-8 border-t border-white/5 mt-8">
                            <button
                                ...
                                className={`shimmer w-full py-6 rounded-2xl text-xl font-black uppercase tracking-[0.2em] shadow-xl transition-all active:scale-[0.98] flex items-center justify-center gap-4  shadow-[var(--ts-accent-color-alpha-20)]
                                    ${(!canAnswer && mode === 'REMOTE') ? 'bg-white/5 text-white/20 cursor-not-allowed' : 'bg-[var(--ts-accent-color)] text-white'}
                                `}
                            >
